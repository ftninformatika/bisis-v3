...
public class DDindikatora extends JDialog {
  ...
  public DDindikatora(Connection conn, String field, Polje fi) {
    try  {
      this.fi=fi;
      this.conn=conn;
      this.field=field;
  ...
  private void jbInit() throws Exception {
    ...
    titledBorder1 = new TitledBorder(border1,Messages.get("DDINDIKATORA_TITLEDBORDER1_FIELD"));
    border2 = BorderFactory.createCompoundBorder(new TitledBorder(BorderFactory.createLineBorder(SystemColor.controlText,1),Messages.get("DDINDIKATORA_LINEBORDER_FIELDS")),BorderFactory.createEmptyBorder(3,3,3,3));
    border3 = BorderFactory.createLineBorder(SystemColor.controlText,1);
    titledBorder2 = new TitledBorder(border3,Messages.get("DDINDIKATORA_TITLEDBORDER2_SECONDINDICATOR"));
    ...
    jLabel1.setText(Messages.get("DDINDIKATORA_LABEL_CODE"));
    this.setResizable(false);
    this.setTitle(Messages.get("DDINDIKATORA_DIALOGTITLE_DEFSECINDICATOR"));
    ...
    jButton10.setText(Messages.get("DDINDIKATORA_BUTTON_INPUT"));
    ...
    jButton11.setText(Messages.get("DDINDIKATORA_BUTTON_CHANGE"));
    ...
    jButton12.setText(Messages.get("DDINDIKATORA_BUTTON_CLEAR"));
    ...
    jButton13.setText(Messages.get("DDINDIKATORA_BUTTON_OK"));
    ...
    jButton14.setText(Messages.get("DDINDIKATORA_BUTTON_CANCEL"));
    ...
    jButton15.setText(Messages.get("DDINDIKATORA_BUTTON_EXIT"));
    ...
    jButton4.setText(Messages.get("DDINDIKATORA_BUTTON_FIRST"));
    ...
    jButton5.setText(Messages.get("DDINDIKATORA_BUTTON_PREVIOUS"));
    ...
    jButton6.setText(Messages.get("DDINDIKATORA_BUTTON_NEXT"));
    ...
    jButton7.setText(Messages.get("DDINDIKATORA_BUTTON_LAST"));
    ...
    jButton8.setText(Messages.get("DDINDIKATORA_BUTTON_VIEW"));
    ...
    jButton9.setText(Messages.get("DDINDIKATORA_BUTTON_HELP"));
    ...
    jLabel4.setText(Messages.get("DDINDIKATORA_LABEL_VALUE"));
    jLabel5.setText(Messages.get("DDINDIKATORA_LABEL_NAME"));
  ...
  void setValues(int row){
    jTextField1.setText((String)dbmDI.getValueAt(row,0));
    jTextArea1.setText((String)dbmDI.getValueAt(row,1));
  }
  ...
  boolean checkValues() {
    if (mod == INSERT) {
      if(jTextField1.getText().length() > 1) {
      	JOptionPane.showMessageDialog(null,Messages.get("DDINDIKATORA_INFDLG_SECONDINDICATOR_MORETHANONECHAR_TEXT"),Messages.get("DDINDIKATORA_INFDLG_SECONDINDICATOR_MORETHANONECHAR_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      	return false;
      }
      if(jTextField1.getText().length() == 0) {
        JOptionPane.showMessageDialog(null,Messages.get("DDINDIKATORA_INFDLG_SECONDINDICATOR_MISSINGSECONDCHAR_TEXT"),Messages.get("DDINDIKATORA_INFDLG_SECONDINDICATOR_MISSINGSECONDCHAR_TITLE"),JOptionPane.INFORMATION_MESSAGE);
        return false;
      }
      if(!(Character.isDigit(jTextField1.getText().charAt(0)))) {
        JOptionPane.showMessageDialog(null,Messages.get("DDINDIKATORA_INFDLG_SECONDINDICATOR_MUSTBENUMERIC_TEXT"),Messages.get("DDINDIKATORA_INFDLG_SECONDINDICATOR_MUSTBENUMERIC_TITLE"),JOptionPane.INFORMATION_MESSAGE);
        return false;
      }
    }
    if(jTextArea1.getText().length() > 160) {
      JOptionPane.showMessageDialog(null,Messages.get("DDINDIKATORA_INFDLG_SECONDINDICATORNAME_LENGTHEXCEEDED_TEXT"),Messages.get("DDINDIKATORA_INFDLG_SECONDINDICATORNAME_LENGTHEXCEEDED_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      return false;
    }
    if(jTextArea1.getText().length() == 0) {
      JOptionPane.showMessageDialog(null,Messages.get("DDINDIKATORA_INFDLG_SECONDINDICATORNAME_NONAME_TEXT"),Messages.get("DDINDIKATORA_INFDLG_SECONDINDICATORNAME_NONAME_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      return false;
    }
    return true;
  }

  void find(String fld,String di) {
    String value = fld+"|"+di;
    int lengths = fld.length()+di.length()+1;
  ...
  void jButton8_actionPerformed(ActionEvent e)  {
    PronadjiDDI di = new PronadjiDDI(this);
    di.pack();
    ...
    di.setLocation((screenSize.width - frameSize.width) / 2, (screenSize.height - frameSize.height) / 2);
    di.setVisible(true);
  }
  ...
  void delete() {
    if(dbmDI.getRowCount() == 1)
      JOptionPane.showMessageDialog(null,Messages.get("DDINDIKATORA_INFDLG_RECORD_LASTRECDELETED_TEXT"),Messages.get("DDINDIKATORA_INFDLG_RECORD_LASTRECDELETED_TITLE"),JOptionPane.INFORMATION_MESSAGE);
    mod = DELETE;
    setButtonEnabled();
    setComponentEnabled();
  }
    ...
    if (mod == INSERT) {
      ...
      if (checkValues()&& pos!=-1) {
        ...
        setComponentEnabled();
      	...
      	if(pos == -1) {
          JOptionPane.showMessageDialog(null,Messages.get("DDINDIKATORA_INFDLG_SECONDINDICATOR_EXISTANCEINDATABASE_TEXT"),Messages.get("DDINDIKATORA_INFDLG_SECONDINDICATOR_EXISTANCEINDATABASE_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      	}
      }
    }
    else if (mod == UPDATE) {
      ...
      dbmDI.removeRow(row);
      if (row == dbmDI.getRowCount() && row!=0) {
        row=row-1;
      }
      if(dbmDI.getRowCount()!=0) {
        trans=true;
  ...
  void doCancel() {
    if(dbmDI.getRowCount() == 0) {
      Vector di = dbmDI.getDrindId();
      fi.reloadDrind(di);
      if(!fi.getCurrDefDI().equals(""))
        if (!di.contains(fi.getCurrDefDI()))
          fi.selectEmptyDI();
      dispose();
    }
    else {
  ...
  protected void processWindowEvent(WindowEvent e){   // ZASTITA OD X-ICA U TOKU NEKE AKCIJE
    if(e.getID() == WindowEvent.WINDOW_CLOSING) {
      if(mod != UNDEFINED) {
        JOptionPane.showMessageDialog(null,Messages.get("DDINDIKATORA_INFDLG_CLOSING_ACTIONNOTFINISHED_TEXT"),Messages.get("DDINDIKATORA_INFDLG_CLOSING_ACTIONNOTFINISHED_TITLE"),JOptionPane.INFORMATION_MESSAGE);
        return;
      }
      else {
