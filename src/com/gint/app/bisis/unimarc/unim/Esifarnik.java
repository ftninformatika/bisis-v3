...
public class Esifarnik extends JDialog {
  ...
  public Esifarnik(Connection conn) {
    try  {
      this.conn=conn;
      jbInit();
    }
  ...
  private void jbInit() throws Exception {
    ...
    titledBorder1 = new TitledBorder(border1,Messages.get("ESIFARNIK_TITLEDBORDER1_EXTERNCODERTYPE"));
    ...
    titledBorder2 = new TitledBorder(border3,Messages.get("ESIFARNIK_TITLEDBORDER2_EXTERNCODER"));
    ...
    jLabel1.setText(Messages.get("ESIFARNIK_LABEL1_CODE"));
    ...
    this.setTitle(Messages.get("ESIFARNIK_DIALOGTITLE_EXTERNCODER"));
    ...
    jButton10.setText(Messages.get("ESIFARNIK_BUTTON_INPUT"));
    ...
    jButton11.setText(Messages.get("ESIFARNIK_BUTTON_CHANGE"));
    ...
    jButton12.setText(Messages.get("ESIFARNIK_BUTTON_CLEAR"));
    ...
    jButton13.setText(Messages.get("ESIFARNIK_BUTTON_OK"));
    ...
    jButton14.setText(Messages.get("ESIFARNIK_BUTTON_CANCEL"));
    ...
    jButton15.setText(Messages.get("ESIFARNIK_BUTTON_EXIT"));
    ...
    jButton4.setText(Messages.get("ESIFARNIK_BUTTON_FIRST"));
    ...
    jButton5.setText(Messages.get("ESIFARNIK_BUTTON_PREVIOUS"));
    ...
    jButton6.setText(Messages.get("ESIFARNIK_BUTTON_NEXT"));
    ...
    jButton7.setText(Messages.get("ESIFARNIK_BUTTON_LAST"));
    ...
    jButton8.setText(Messages.get("ESIFARNIK_BUTTON_VIEW"));
    ...
    jButton9.setText(Messages.get("ESIFARNIK_BUTTON_HELP"));
    ...
    jLabel4.setText(Messages.get("ESIFARNIK_LABEL4_CODE"));
    jLabel5.setText(Messages.get("ESIFARNIK_LABEL_NAME"));
    ...
    jPanel5.add(jButton6, new XYConstraints(229, 3, 0, 0));
    jPanel5.add(jButton9, new XYConstraints(565, 3, 0, 0));
    jPanel5.add(jPanel7, new XYConstraints(336, 30, 204, -1));
    jPanel7.add(jButton14, new XYConstraints(115, 1, 0, 0));
    jPanel7.add(jButton13, new XYConstraints(2, 1, 0, 0));
    jPanel5.add(jButton15, new XYConstraints(565, 36, 0, 0));
    jPanel5.add(jButton7, new XYConstraints(341, 3, 0, 0));
    jPanel5.add(jButton5, new XYConstraints(117, 3, 0, 0));
    jPanel5.add(jButton8, new XYConstraints(453, 3, 0, 0));
    jPanel5.add(jButton4, new XYConstraints(5, 3, 0, 0));
    jPanel5.add(jPanel6, new XYConstraints(0, 30, 315, -1));
    ...
    trans=true;
    dbmES = new DBMESifarnik(conn);

    dbcm1 = new DBComboModel(conn,"select distinct TESID,TES_NAZIV from tipekst_sifarnika order by TESID",true);
    jComboBox1.setModel(dbcm1);
    trans=false;

    if (dbmES.getRowCount() != 0){
      trans=true;
      setValues(row);
      setComponentEnabled();
  ...
  void setValues(int row){
    jComboBox1.setSelectedItem((String)dbmES.getValueAt(row,0));
    jTextField1.setText((String)dbmES.getValueAt(row,1));
    jTextArea1.setText((String)dbmES.getValueAt(row,2));
  }
  ...
  boolean checkValues() {
    if (mod == INSERT) {
      if(jTextField1.getText().length() > 15) {
      	JOptionPane.showMessageDialog(null,Messages.get("ESIFARNIK_INFDLG_EXTERNCODER_MORETHAN15CHAR_TEXT"),Messages.get("ESIFARNIK_INFDLG_EXTERNCODER_MORETHAN15CHAR_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      	return false;
      }
      if(jTextField1.getText().length() == 0) {
        JOptionPane.showMessageDialog(null,Messages.get("ESIFARNIK_INFDLG_EXTERNCODER_MISSINGEXTERNCODER_TEXT"),Messages.get("ESIFARNIK_INFDLG_EXTERNCODER_MISSINGEXTERNCODER_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      	return false;
      }
    }
    if(jTextArea1.getText().length() > 160) {
      JOptionPane.showMessageDialog(null,Messages.get("ESIFARNIK_INFDLG_EXTERNCODERNAME_LENGTHEXCEEDED_TEXT"),Messages.get("ESIFARNIK_INFDLG_EXTERNCODERNAME_LENGTHEXCEEDED_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      return false;
    }
    if(jTextArea1.getText().length() == 0) {
      JOptionPane.showMessageDialog(null,Messages.get("ESIFARNIK_INFDLG_EXTERNCODERNAME_NONAME_TEXT"),Messages.get("ESIFARNIK_INFDLG_EXTERNCODERNAME_NONAME_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      return false;
    }
    return true;
  }

  void find(String tes,String eSfr) {
    String value = tes+"|"+eSfr;
    int lengths = tes.length()+eSfr.length()+1;
    int pos = dbmES.find(value,lengths);
    if (pos!=-1){
      row=pos;
      trans=true;
      setValues(row);
  ...
  public String getTESifarnik() {
    return (String)dbmES.getValueAt(row,0);
  }
  public String getESifarnik() {
    return (String)dbmES.getValueAt(row,1);
  }
  void first() {
    trans=true;
    row=0;
  ...
  void jButton8_actionPerformed(ActionEvent e)  {
    PronadjiES pes = new PronadjiES(conn,this);
    pes.pack();
    Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
    Dimension frameSize = pes.getSize();
    if (frameSize.height > screenSize.height)
      frameSize.height = screenSize.height;
    if (frameSize.width > screenSize.width)
      frameSize.width = screenSize.width;
    pes.setLocation((screenSize.width - frameSize.width) / 2, (screenSize.height - frameSize.height) / 2);
    pes.setVisible(true);
  }
  ...
  void delete() {
    if(dbmES.getRowCount() == 1)
      JOptionPane.showMessageDialog(null,Messages.get("ESIFARNIK_INFDLG_RECORD_LASTRECDELETED_TEXT"),Messages.get("ESIFARNIK_INFDLG_RECORD_LASTRECDELETED_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      mod = DELETE;
      setButtonEnabled();
      setComponentEnabled();
  }
  ...
  void doAction() {
    ...
    String eSfr = jTextField1.getText().trim();
    String naziv = jTextArea1.getText().trim();
    dbmES.setTESifarnik(tes);
    dbmES.setESifarnik(eSfr);
    dbmES.setNazivESifarnik(naziv);
    if (mod == INSERT) {
      int pos = dbmES.findPos(tes,eSfr);
      if (checkValues()&& pos!=-1) {
        row = pos;
        dbmES.setTo(row,mod);
        dbmES.saveToDB(mod,row);
        if(isEmpty)
          isEmpty=false;
        mod = UNDEFINED;
        setButtonEnabled();
        ...
        jButton6.requestFocus();
      	...
      	if(pos == -1) {
          JOptionPane.showMessageDialog(null,Messages.get("ESIFARNIK_INFDLG_EXTERNCODER_EXISTANCEINDATABASE_TEXT"),Messages.get("ESIFARNIK_INFDLG_EXTERNCODER_EXISTANCEINDATABASE_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      	}
      }
    }
    else if (mod == UPDATE) {
  ...
  void jComboBox1_actionPerformed(ActionEvent e)  {
    ...
    if(!trans) {
      trans=true;
      String tes = ((String)jComboBox1.getSelectedItem()).trim();
      //int i = dbmES.find(tes,tes.length());
      int i = dbmES.find(tes);
      if(i != -1) {
        row = i;
        setValues(row);
        jButton6.requestFocus();
  ...
  protected void processWindowEvent(WindowEvent e){   // ZASTITA OD X-ICA U TOKU NEKE AKCIJE
    if(e.getID() == WindowEvent.WINDOW_CLOSING) {
      if(mod != UNDEFINED) {
        JOptionPane.showMessageDialog(null,Messages.get("ESIFARNIK_INFDLG_CLOSING_ACTIONNOTFINISHED_TEXT"),Messages.get("ESIFARNIK_INFDLG_CLOSING_ACTIONNOTFINISHED_TITLE"),JOptionPane.INFORMATION_MESSAGE);
        return;
      }
      else {
