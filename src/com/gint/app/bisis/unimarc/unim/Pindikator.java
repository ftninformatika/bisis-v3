...
public class Pindikator extends JDialog {
  ...
  public Pindikator(Connection conn) {
    try  {
      this.conn=conn;
      jbInit();
    }
  ...
  private void jbInit() throws Exception {
    ...
    titledBorder1 = new TitledBorder(border1,Messages.get("PINDIKATOR_TITLEDBORDER1_FIELDS"));
    ...
    titledBorder2 = new TitledBorder(border3,Messages.get("PINDIKATOR_TITLEDBORDER2_FIRSTINDICATOR"));
    ...
    jLabel1.setText(Messages.get("PINDIKATOR_LABEL_CODE"));
    ...
    this.setTitle(Messages.get("PINDIKATOR_DIALOGTITLE_FIRSTINDICATOR"));
    ...
    jButton10.setText(Messages.get("PINDIKATOR_BUTTON_INPUT"));
    ...
    jButton11.setText(Messages.get("PINDIKATOR_BUTTON_CHANGE"));
    ...
    jButton12.setText(Messages.get("PINDIKATOR_BUTTON_CLEAR"));
    ...
    jButton13.setText(Messages.get("PINDIKATOR_BUTTON_OK"));
    ...
    jButton14.setText(Messages.get("PINDIKATOR_BUTTON_CANCEL"));
    ...
    jButton15.setText(Messages.get("PINDIKATOR_BUTTON_EXIT"));
    ...
    jButton4.setText(Messages.get("PINDIKATOR_BUTTON_FIRST"));
    ...
    jButton5.setText(Messages.get("PINDIKATOR_BUTTON_PREVIOUS"));
    ...
    jButton6.setText(Messages.get("PINDIKATOR_BUTTON_NEXT"));
    ...
    jButton7.setText(Messages.get("PINDIKATOR_BUTTON_LAST"));
    ...
    jButton8.setText(Messages.get("PINDIKATOR_BUTTON_VIEW"));
    ...
    jButton9.setText(Messages.get("PINDIKATOR_BUTTON_HELP"));
    ...
    jLabel4.setText(Messages.get("PINDIKATOR_LABEL_VALUE"));
    jLabel5.setText(Messages.get("PINDIKATOR_LABEL_NAME"));
    ...
    jPanel4.add(jLabel1, new XYConstraints(5, 3, -1, -1));
  ...
  void setValues(int row){
    jComboBox1.setSelectedItem((String)dbmPI.getValueAt(row,0));
    jTextField1.setText((String)dbmPI.getValueAt(row,1));
    jTextArea1.setText((String)dbmPI.getValueAt(row,2));
  }
  void setName() {
  ...
  boolean checkValues() {
    if (mod == INSERT) {
      if(jTextField1.getText().length() > 1) {
      	JOptionPane.showMessageDialog(null,Messages.get("PINDIKATOR_INFDLG_FIRSTINDICATOR_MORETHANONECHAR_TEXT"),Messages.get("PINDIKATOR_INFDLG_FIRSTINDICATOR_MORETHANONECHAR_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      	return false;
      }
      if(jTextField1.getText().length() == 0) {
        JOptionPane.showMessageDialog(null,Messages.get("PINDIKATOR_INFDLG_FIRSTINDICATOR_MISSINGFIRSTCHAR_TEXT"),Messages.get("PINDIKATOR_INFDLG_FIRSTINDICATOR_MISSINGFIRSTCHAR_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      	return false;
      }
      if(!(Character.isDigit(jTextField1.getText().charAt(0)))) {
      	JOptionPane.showMessageDialog(null,Messages.get("PINDIKATOR_INFDLG_FIRSTINDICATOR_MUSTBENUMERIC_TEXT"),Messages.get("PINDIKATOR_INFDLG_FIRSTINDICATOR_MUSTBENUMERIC_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      	return false;
      }
    }
    if(jTextArea1.getText().length() > 160) {
      JOptionPane.showMessageDialog(null,Messages.get("PINDIKATOR_INFDLG_FIRSTINDICATORNAME_LENGTHEXCEEDED_TEXT"),Messages.get("PINDIKATOR_INFDLG_FIRSTINDICATORNAME_LENGTHEXCEEDED_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      return false;
    }
    if(jTextArea1.getText().length() == 0) {
      JOptionPane.showMessageDialog(null,Messages.get("PINDIKATOR_INFDLG_FIRSTINDICATORNAME_NONAME_TEXT"),Messages.get("PINDIKATOR_INFDLG_FIRSTINDICATORNAME_NONAME_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      return false;
    }
    return true;
  }

  void find(String fld,String prind) {
    String value = fld+"|"+prind;
    int lengths = fld.length()+prind.length()+1;
  ...
  public String getPrind() {
    return (String)dbmPI.getValueAt(row,1);
  }
  ...
  void jButton8_actionPerformed(ActionEvent e)  {
    PronadjiPI ppi = new PronadjiPI(conn,this);
    ppi.pack();
    Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();
    Dimension frameSize = ppi.getSize();
    if (frameSize.height > screenSize.height)
      frameSize.height = screenSize.height;
    if (frameSize.width > screenSize.width)
      frameSize.width = screenSize.width;
    ppi.setLocation((screenSize.width - frameSize.width) / 2, (screenSize.height - frameSize.height) / 2);
    ppi.setVisible(true);
  }
  ...
  void delete() {
    if(dbmPI.getRowCount() == 1)
      JOptionPane.showMessageDialog(null,Messages.get("PINDIKATOR_INFDLG_RECORD_LASTRECDELETED_TEXT"),Messages.get("PINDIKATOR_INFDLG_RECORD_LASTRECDELETED_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      mod = DELETE;
      setButtonEnabled();
      setComponentEnabled();
  }
  ...
  void doAction() {
    ...
    String prind = jTextField1.getText().trim();
    String naziv = jTextArea1.getText().trim();
    dbmPI.setField(fld);
    dbmPI.setPrind(prind);
    dbmPI.setNazivPrind(naziv);
    if (mod == INSERT) {
      int pos = dbmPI.findPos(fld,prind);
      if (checkValues()&& pos!=-1) {
        row = pos;
        dbmPI.setTo(row,mod);
        dbmPI.saveToDB(mod,row);
        if(isEmpty)
          isEmpty=false;
        mod = UNDEFINED;
        setButtonEnabled();
        ...
        jButton6.requestFocus();
      	...
      	if(pos == -1) {
          JOptionPane.showMessageDialog(null,Messages.get("PINDIKATOR_INFDLG_FIRSTINDICATOR_EXISTANCEINDATABASE_TEXT"),Messages.get("PINDIKATOR_INFDLG_FIRSTINDICATOR_EXISTANCEINDATABASE_TITLE"),JOptionPane.INFORMATION_MESSAGE);
      	}
      }
    }
    else if (mod == UPDATE) {
  ...
  void jComboBox1_actionPerformed(ActionEvent e)  {
    ...
    if(!trans) {
      trans=true;
      String fld = ((String)jComboBox1.getSelectedItem()).trim();
      int i = dbmPI.find(fld,fld.length());
      if(i != -1) {
        row = i;
        setValues(row);
        jButton6.requestFocus();
  ...
  protected void processWindowEvent(WindowEvent e){   // ZASTITA OD X-ICA U TOKU NEKE AKCIJE
    if(e.getID() == WindowEvent.WINDOW_CLOSING) {
      if(mod != UNDEFINED) {
        JOptionPane.showMessageDialog(null,Messages.get("PINDIKATOR_INFDLG_CLOSING_ACTIONNOTFINISHED_TEXT"),Messages.get("PINDIKATOR_INFDLG_CLOSING_ACTIONNOTFINISHED_TITLE"),JOptionPane.INFORMATION_MESSAGE);
        return;
      }
      else {
