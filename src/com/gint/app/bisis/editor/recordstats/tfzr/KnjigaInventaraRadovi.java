package com.gint.app.bisis.editor.recordstats.tfzr;
...
public class KnjigaInventaraRadovi extends RecordReport {

  public void execute() {
    try {
      startNumber = ((Integer)params.get("startDate")).intValue(); 
      endNumber = ((Integer)params.get("endDate")).intValue();
...
//      in.close();
      ...
      JasperPrint jp = JasperFillManager.fillReport(
          KnjigaInventaraRadovi.class.getResource(
            "/com/gint/app/bisis/editor/recordstats/tfzr/KnjigaInventaraRadovi.jasper")
  ...
  public void handleRecord(Record rec) {
    ...
    if (type == null || !type.startsWith("009"))
      return;
    ...
    while (it.hasNext()) {
      ...
      String invbr = sf996f.getContent();
      int nInvbr = Integer.parseInt(invbr);

      if (nInvbr > endNumber || nInvbr < startNumber)
        continue;

      Subfield testSig = f.getSubfield('d');
      if (testSig != null) {
        sig = " ";
        Subsubfield ssfl = testSig.getSubsubfield('l');
        Subsubfield ssfn = testSig.getSubsubfield('n');
        if (ssfl != null)
          sig = ssfl.getContent();
        if (ssfn != null) {
          if (!" ".equals(sig))
