...
public class ChangeTemplate extends JPanel {
	...
	public ChangeTemplate(DBConnection dbconn) {
		super();
		initialize();
		this.dbconn = dbconn;
	}
...
//				    jTree.setCellRenderer(renderer);
	...
	public void setContent(int id){
		...
		try{
			String wtext = null;
			String qry = "select wtext from warning_types where id = " + id;
			Statement stmt = dbconn.getConnection().createStatement();
			ResultSet rset = stmt.executeQuery(qry);
			if (rset.next()){
				wtext = rset.getString(1);
				
				doc = RootDocument.Factory.parse(wtext);
				...
				getTfCena1().setText(doc.getRoot().getOpomenaArray(0).getBody().getCena1().toString());
				getTfCena2().setText(doc.getRoot().getOpomenaArray(0).getBody().getCena2().toString());
	...
	public void save(){
		try{
			...
			doc.getRoot().getOpomenaArray(0).getBody().setCena1(new BigDecimal(Double.parseDouble(getTfCena1().getText())));
			doc.getRoot().getOpomenaArray(0).getBody().setCena2(new BigDecimal(Double.parseDouble(getTfCena2().getText())));
			...
			doc.save(sw);
			try{
				String qry = "update warning_types set wtext = '"+ sw.toString() +"' where id = " + id;
				Statement stmt = dbconn.getConnection().createStatement();
