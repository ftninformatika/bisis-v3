...
public class PrefixListModel extends AbstractListModel {
  ...
  public PrefixListModel(Connection conn) {
    this((Locale)null, conn);
  }
  
  public PrefixListModel(String locale, Connection conn) {
    this(new Locale(locale), conn);
  }

  public PrefixListModel(Locale locale, Connection conn) {
...
//          allPrefixes.addElement(pref);
          displayed.add(Charset.YUSCII2Unicode(rset.getString(1).trim()) + " - " + Charset.YUSCII2Unicode(rset.getString(2)));
        }
        rset.close();
        stmt.close();
      }
